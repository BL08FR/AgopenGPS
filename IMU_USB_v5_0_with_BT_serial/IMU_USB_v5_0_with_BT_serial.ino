  ...
  */
  ...
  { 
    //BTmod setup
    Serial.begin(115200);
    SerialBT.begin("IMU_ESP32_BT"); //Bluetooth device name
    Serial.println("The device started, now you can pair it with bluetooth!");SerialBT.println("The device started, now you can pair it with bluetooth!");
    delay(10000);
    
    //Serial.begin(38400);  // Start serial port (disabled to use higher speed in BTmod)
    Wire.begin();

    //test if CMPS working
    uint8_t error;
    Serial.println("Checking for CMPS14");SerialBT.println("Checking for CMPS14");
  ...
  { 
    ...
    SerialBT.write(data, dataSize); //BTmod write the same on BTserail

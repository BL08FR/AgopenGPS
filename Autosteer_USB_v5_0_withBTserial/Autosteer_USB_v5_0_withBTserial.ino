...
/////////////////////////////////////////////
  ...
  //Serial switch variables
  int serialSWtimer = 2000;
  int BTtesttimer = 2000;
  long BTtesttime;
  long serialSWtime;
  boolean ACTserial = 0;
  boolean PREVserial = 0;
...
/*
  // Serial switch code
  if(millis()-BTtesttime >= BTtesttimer){// test every X seconds
    PREVserial = ACTserial;// actual value become previous
    if(SerialBT.available() > 0){ACTserial = 1;}// test if BT serial is active
    else{ACTserial = 0;}// if no, set actual serial is USB
  }
  
  if(ACTserial == 1 && PREVserial == 0){serialSWtime = millis();}// test if active serial changed, if yes set time for timer
  if(ACTserial == 0 && PREVserial == 1){serialSWtime = millis();}
  
  if(ACTserial == 1 && BT == 0 && millis()-serialSWtime >= serialSWtimer){BT = 1;}// if timer time exceeded switch serial mode if prev & actual are different
  if(ACTserial == 0 && BT == 1 && millis()-serialSWtime >= serialSWtimer){BT = 0;}
*/
  
   if(BT == 0){ 
    //SerialBT.println("BT == 0 loop!"); //for test
  ...
  if(BT == 1){ //bluetooth loop
    //SerialBT.println("BT == 1 loop"); //for test

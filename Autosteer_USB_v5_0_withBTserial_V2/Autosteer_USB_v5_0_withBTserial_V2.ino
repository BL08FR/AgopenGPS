...
/////////////////////////////////////////////
  ...
  #define PWM2_RPWM  5 //D9 a l'origine
  ...
  SoftwareSerial SerialBT(12, 11);//start serial connection
  ...
  { 
    ...
    Wire.begin();
    Serial.begin(38400);SerialBT.begin(38400);
    ...
    if (error == 0)
    {
      Serial.println("Error = 0");SerialBT.println("Error = 0");
      Serial.print("CMPS14 ADDRESs: 0x");SerialBT.print("CMPS14 ADDRESs: 0x");
      Serial.println(CMPS14_ADDRESS, HEX);SerialBT.println(CMPS14_ADDRESS, HEX);
      Serial.println("CMPS14 Ok.");SerialBT.println("CMPS14 Ok.");

      useCMPS = true;
    }
    else 
    {
      Serial.println("Error = 4");SerialBT.println("Error = 4");
      Serial.println("CMPS not Connected or Found");SerialBT.println("CMPS not Connected or Found");
      useCMPS = false;
    }
    ...
    if(!useCMPS)
    {
      for(int16_t i = 0; i < nrBNO08xAdresses; i++)
      {
        ...
        Serial.print("\r\nChecking for BNO08X on ");SerialBT.print("\r\nChecking for BNO08X on ");
        Serial.println(bno08xAddress, HEX);SerialBT.println(bno08xAddress, HEX);
        ...
        {
          Serial.println("Error = 0");SerialBT.println("Error = 0");
          Serial.print("BNO08X ADDRESs: 0x");SerialBT.print("BNO08X ADDRESs: 0x");
          Serial.println(bno08xAddress, HEX);SerialBT.println(bno08xAddress, HEX);
          Serial.println("BNO08X Ok.");SerialBT.println("BNO08X Ok.");
          ...
          if (bno08x.begin(bno08xAddress))
          {
            ...
            {
              Serial.println("BNO08x init fails!!");SerialBT.println("BNO08x init fails!!");
            }
          }
          else
          {
            Serial.println("BNO080 not detected at given I2C address.");SerialBT.println("BNO080 not detected at given I2C address.");
          }
        }
        else 
        {
          Serial.println("Error = 4");SerialBT.println("Error = 4");
          Serial.println("BNO08X not Connected or Found"); SerialBT.println("BNO08X not Connected or Found"); 
        }
      }
    }
  ...
  void loop()
  {
      ...
      if (++helloCounter > 10)
      {
        Serial.write(helloAgIO,sizeof(helloAgIO));SerialBT.write(helloAgIO,sizeof(helloAgIO));
        helloCounter = 0;
      }
    } //end of timed loop
    ...
    if (Serial.available() > 1 && !isHeaderFound && !isPGNFound)  // USB serial situation
    {
    ...
    if (SerialBT.available() > 1 && !isHeaderFound && !isPGNFound) // BT serial situation
    {
    ...
    if (Serial.available() > 2 && isHeaderFound && !isPGNFound)// USB serial situation
    {
    ...
    if (SerialBT.available() > 2 && isHeaderFound && !isPGNFound)// BT serial situation
    {
    ...
    //The data package
    if (Serial.available() > dataLength && isHeaderFound && isPGNFound) // USB serial situation
    {
    ...
    if (SerialBT.available() > dataLength && isHeaderFound && isPGNFound)
    {
      if (pgn == 254) //FE AutoSteerData
      {
        ...
        //bit 7
        guidanceStatus = SerialBT.read();

        //Bit 8,9    set point steer angle * 100 is sent
        steerAngleSetPoint = ((float)(SerialBT.read()| SerialBT.read() << 8 ))*0.01; //high low bytes
      ...
      {         
        //PID values
        steerSettings.Kp = ((float)SerialBT.read());   // read Kp from AgOpenGPS
        steerSettings.Kp*=0.5;
        
        steerSettings.highPWM = SerialBT.read();
        
        steerSettings.lowPWM = (float)SerialBT.read();   // read lowPWM from AgOpenGPS
                
        steerSettings.minPWM = SerialBT.read(); //read the minimum amount of PWM for instant on
        
        steerSettings.steerSensorCounts = SerialBT.read(); //sent as setting displayed in AOG
        
        steerSettings.wasOffset = (SerialBT.read());  //read was zero offset Hi
               
        steerSettings.wasOffset |= (SerialBT.read() << 8);  //read was zero offset Lo
        
        steerSettings.AckermanFix = (float)SerialBT.read() * 0.01; 
      ...
      {       
        ...
        steerConfig.PulseCountMax = SerialBT.read();

        //was speed
        SerialBT.read(); 
        
        sett = SerialBT.read(); //byte 8 - setting1 - Danfoss valve etc

        if (bitRead(sett, 0)) steerConfig.IsDanfoss = 1; else steerConfig.IsDanfoss = 0;
        if (bitRead(sett, 1)) steerConfig.PressureSensor = 1; else steerConfig.PressureSensor = 0;
        if (bitRead(sett, 2)) steerConfig.CurrentSensor = 1; else steerConfig.CurrentSensor = 0;
              
        SerialBT.read(); //byte 9
        SerialBT.read(); //byte 10
         
        SerialBT.read(); //byte 11
        SerialBT.read(); //byte 12
    ...
    } //end if (SerialBT.available() > dataLength && isHeaderFound && isPGNFound) 
